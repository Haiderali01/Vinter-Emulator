<?php
	...
	while (1) {
		...
		for ($i=0; $i<count($schedule_files); $i++) {
			...
			$obj_driver->waitfor(5);
			$obj_driver->executeCMD("rsync --remove-source-files -avz -e 'sshpass -f /opt/ssh_pass ssh -p 22' /var/www/app_ad_images/ root@139.162.196.166:/vinter_temp_images/");
	...
	{
		$status_url = "http://139.162.196.166/social/script_status.php?conf_device_id=".$config_id;
			echo $status_url;
