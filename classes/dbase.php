...
*/	}
	...
	function Dbase($db_nsme="", $db_host="",$db_user="",$db_pass=""){
		...
		if($db_host){
                        ...
                        $this->MYSQL_HOST = "178.79.178.252";
                }
                if($db_user){
                        $this->USER_NAME = $db_user;
                }else{
		...
		if ($this->connectHost()) {
			if (!($this->connectDB())) {
				exit("There is some problem with Database Connections, please contact ".$this->site." administrator! <br>".$this->DATABASE."<br>".mysqli_error());
			}
		} else {
			exit("There is some problem with Database Connections, please contact ".$this->site." administrator! <br>".$this->MYSQL_HOST."<br>".mysqli_error());
		}
	...
	function connectHost() {
		...
		$this->dbLink = mysqli_connect($this->MYSQL_HOST, $this->USER_NAME, $this->USER_PWD, $this->DATABASE) or die("CONNECTION ERROR: ".mysqli_error());
		if (!$this->dbLink) {
			return false;
		}
		return true;
