<?php
	...
	class Android_Vinter_Driver {
		...
		//	$this->executeOnDevice("shell screencap -p | sed 's/\r$//' > ".$fileame);
		...
		function executeOnDevice($cmd) {
			...
			return trim(implode("\n", $this->executeCMD($this->adbPath." -s ".$this->configs->device_adb_id." ".$cmd)));
		}
		...
		function currentActivity() {
			$output = $this->executeOnDevice("shell dumpsys activity activities | grep 'Hist #'|head -n 1");
			/*$ex = explode(" ", $output);
			$activity_name = explode("/", str_replace("}", "", $ex[count($ex)-1]));
			return str_replace($this->configs->app_package, "", $activity_name[1]);*/
			$output2  = explode(" " , $output);
			$current_activity = trim($output2[count($output2) - 2]);
			echo "\n ========  current activity =============\n";
			echo "\n".$current_activity."\n";
			$current_activity2 = explode("/",$current_activity);
				echo "\n ========  current activity =============\n";

			return str_replace($this->configs->app_package, "", $current_activity2[1]);
