	...
	class Android_Vinter_Driver {
		...
		function takeSaveScreenshot($blackscreen=true) {
			...
			/*if ($blackscreen) {
				...
				$image_mobile_location = '/data/local/tmp/veri_images/';

				$this->executeOnDevice("shell rm -rf /data/local/tmp/veri_images/");

				$this->executeOnDevice("shell mkdir /data/local/tmp/veri_images/");
				$this->executeOnDevice("shell chmod -R 777 /data/local/tmp/veri_images/");

			$date_folder = $image_mobile_location.date("Y-m-d")."/";

			$this->executeOnDevice("shell mkdir $date_folder");

			$this->executeOnDevice("shell chmod -R 777 $date_folder");

			$hour_folder = $date_folder.date("H")."/";

			$this->executeOnDevice("shell mkdir $hour_folder");

			$this->executeOnDevice("shell chmod -R 777 $hour_folder");
			...
			$this->executeOnDevice("pull $image_mobile_location ".$this->imagesLocation."");


			$this->executeOnDevice("shell rm -rf $image_mobile_location");


			$fileame = str_replace($image_mobile_location, $this->imagesLocation, $fileame);
